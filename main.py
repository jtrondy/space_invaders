...
class Player(Ship):  # inherits ship
    def __init__(self, x, y, health=100):
...
def main():
    ...
    while run:
        ...
        if keys[pygame.K_s] and player.y + player_velocity + player.get_height() + 20 < HEIGHT:  # move down
            player.y += player_velocity
        if keys[pygame.K_SPACE]:
            player.shoot()

        for enemy in enemies[:]:  # [:] means to copy the list
